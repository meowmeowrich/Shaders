...
uniform sampler2D colortex6; // History
...
void main() {
    // Checkerboard update for 2x performance gain
    ...
    // Performance Raymarching (low step count, blue noise)
    float jitter = blueNoise(texCoord, frameCounter);
    vec3 rayDir = normalize(normal + hash33(vec3(texCoord, jitter)) * 2.0 - 1.0);
    if (rayDir.z > 0.0) rayDir.z *= -1.0;

    vec3 indirect = vec3(0.0);
    vec3 p = viewPos + rayDir * 0.2;

    // Only 6 steps for massive performance boost, relying on temporal accumulation
    for(int i = 0; i < 6; i++) {
        ...
        if (p.z < hitPos.z && length(p - hitPos) < 1.0) {
            indirect = texture2D(colortex0, screen.xy).rgb * 0.8 * GI_BOUNCE_INTENSITY;
            break;
        }
        p += rayDir * 2.0;
    }

    vec3 history = texture2D(colortex6, texCoord).rgb;
    outPathTrace = vec4(mix(history, indirect, 0.1), 1.0);
}
